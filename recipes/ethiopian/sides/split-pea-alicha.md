---
name: Split Pea Alicha
culture: Ethiopian Inspired
category: Sides
status: First Test
difficulty: Easy
prep_time: 15m
cook_time: 40-60m
total_time: 1h 15m
servings: 2-4 side servings
measurement_status: tested-draft
rating_note: Practical yellow split pea side for the Doro Wat platter
repeat_worthy: yes
hall_of_fame_candidate: false
hall_of_fame: false
tags:
  - ethiopian-inspired
  - split-peas
  - alicha
  - side-dish
  - stew
  - dairy-minimal
  - butter-ok
  - doro-wat-platter
pairs_well_with:
  - Ethiopian-Inspired Doro Wat Platter
last_updated: 2026-06-16
---

<!-- recipe-card:start -->

# Split Pea Alicha

> A mild golden split pea stew designed as the practical, repeatable side for the Ethiopian-inspired Doro Wat platter.

## Recipe Dashboard

| Status | Total Time | Servings | Difficulty |
|---|---:|---:|---|
| First Test | 1h 15m | 2-4 side servings | Easy |

| Prep Time | Cook Time | Rating Note | Repeat Worthy |
|---:|---:|---|---|
| 15m | 40-60m | Practical yellow split pea side for the Doro Wat platter | Yes |

## Ingredient List

| Component | Ingredients |
|---|---|
| Split Pea Base | Yellow split peas, water or broth |
| Aromatic Base | Onion, unsalted butter or oil, garlic, ginger |
| Spices | Turmeric, cumin, coriander, black pepper, optional cayenne or berbere |
| Finish | Salt, optional lemon |

## Measured Ingredients

### Split Pea Alicha

| Amount | Ingredient | Notes |
|---:|---|---|
| 1 cup | Yellow split peas | Rinsed well; green split peas are backup only |
| 3-3 1/2 cups | Water or broth | Plus more as needed |
| 1 small | Onion | Finely diced |
| 1-2 Tbsp | Unsalted butter or oil | Butter is acceptable |
| 2 cloves | Garlic | Minced |
| 1 tsp | Fresh grated ginger or ginger paste |  |
| 1/2 tsp | Turmeric |  |
| 1/2 tsp | Cumin |  |
| 1/2 tsp | Coriander |  |
| 1/4 tsp | Black pepper |  |
| Optional | Cayenne or berbere | Small pinch for gentle warmth |
| To taste | Salt | Add near the end |
| Optional | Lemon juice | Small squeeze only if heavy or flat |

## Instructions

| Step | Action |
|---:|---|
| 1 | Rinse the split peas in a fine mesh strainer until the water runs mostly clear. Pick out debris or damaged pieces. |
| 2 | Melt butter or heat oil in a saucepan or small Dutch oven. Add onion and cook 8-12 minutes until softened, reduced, and lightly golden. Add tiny splashes of water if needed to prevent sticking. |
| 3 | Add garlic, ginger, turmeric, cumin, coriander, black pepper, and optional cayenne or berbere. Cook 30-60 seconds, stirring constantly, until fragrant. Do not burn the spices. |
| 4 | Add rinsed split peas and 3 cups water or broth. Stir well and scrape the bottom of the pot. Bring to a simmer. |
| 5 | Simmer uncovered or partially covered for 40-60 minutes, stirring occasionally, until the peas are very tender and beginning to break down. Add more liquid as needed. |
| 6 | Mash some peas against the side of the pot to create a creamy body. For a creamier stew, mash 1/3 to 1/2 of the mixture. |
| 7 | Continue simmering until thick, spoonable, and stew-like. Season with salt. Add lemon only if the stew tastes heavy or flat. |

<!-- recipe-card:end -->

## Pairs Well With

| Type | Pairing | Notes |
|---|---|---|
| Main plate | [Ethiopian-Inspired Doro Wat Platter](../bowls-and-mains/ethiopian-inspired-doro-wat-platter.md) | Designed as the practical mild side for the platter. |

## Chef's Notes

Yellow split peas are the preferred version. They cook into a creamy golden stew and provide a mild counterpoint to the spicy berbere-style chicken.

Green split peas are a backup. They work with the same method, but the flavor will be earthier and the color will be darker green-brown.

Avoid forcing brown or green whole lentils if split peas are available. Whole lentils hold their shape more and do not create the same creamy stew texture.

## Final Verdict

**Worth making again?**

Yes. This is the practical working side for the first Ethiopian-inspired platter.

**Flavorweaver Decision:**

- [x] Keep as written
- [ ] Keep with changes
- [ ] Move toward Hall of Fame
- [ ] Mark as House Staple
- [x] Keep for testing
- [ ] Do not repeat

<details>
<summary>Flavor Architecture</summary>

| Layer | Notes |
|---|---|
| Richness | Butter or oil; creamy peas |
| Freshness | Optional lemon finish |
| Acid / Brightness | Lemon only if needed |
| Heat | Optional cayenne or berbere |
| Sweetness | Cooked onion and yellow split peas |
| Umami | Onion, garlic, broth if used |
| Texture | Thick, spoonable, creamy, some pea texture remaining |
| Sauce / Moisture | Stew-like body |
| Aroma | Ginger, turmeric, cumin, coriander |

</details>

<details>
<summary>Flavorweaver Direction</summary>

This is the accessible platter side. Use it in the main Doro Wat platter card while preserving Misir Wat as the red lentil traditional direction.

</details>

<details>
<summary>Timing Notes</summary>

Optional soaking can reduce cooking time and help the peas soften more evenly. Quick soak 2-4 hours, then drain and rinse. No-soak works but may need more simmering time and liquid.

</details>

<details>
<summary>Testing Notes</summary>

## First Test

**Date:** 2026-06-14

**What worked:**

- Yellow split peas solve the red lentil sourcing issue.
- Mild flavor balances the spicy doro wat-style chicken.
- Creamy texture fits the platter better than whole lentils.

**What to adjust next time:**

- Confirm exact cook time for the specific split peas used.
- Decide whether broth or water gives the preferred flavor.

</details>

<details>
<summary>Kitchen Notes</summary>

If the peas stay firm, keep simmering and add more water or broth. If watery, simmer uncovered and mash more peas. If flat, add salt first, then lemon only if needed.

</details>

<details>
<summary>Change Log</summary>

| Date | Change |
|---|---|
| 2026-06-14 | Created source file from Ethiopian Split Pea Alicha handoff. |
| 2026-06-16 | Removed Misir Wat from Pairs Well With; substitution context belongs in the main Doro Wat platter note. |

</details>
