---
name: Fresh Lemongrass Chicken Spring Rolls
culture: Vietnamese Inspired
primary_category: Appetizer
categories:
  - Appetizer
  - Side
status: Working Recipe
difficulty: Moderate
prep_time: 40m
cook_time: 20m
marinate_time: 2-4h
total_time: 3h-5h
servings:
measurement_status: partial

repeat_worthy:
hall_of_fame_candidate: false
hall_of_fame: false

tags:
  - spring rolls
  - fresh rolls
  - lemongrass chicken
  - vietnamese-inspired
  - rice paper
  - peanut sauce
  - ginger nuoc cham
  - fresh vegetables
  - appetizer
  - side
  - multi-component

pairs_well_with:
  - peanut-sauce
  - ginger-nuoc-cham
  - lemongrass-chicken-bowls

last_updated: 2026-06-11
---

<!-- recipe-card:start -->

# Fresh Lemongrass Chicken Spring Rolls

> Fresh rice paper spring rolls filled with sliced lemongrass chicken, napa cabbage, cucumber, carrot, cilantro, and cooled angel hair pasta, served with peanut sauce and ginger nuoc cham.

## Recipe Dashboard

| Status | Total Time | Servings | Difficulty |
|---|---:|---:|---|
| Working Recipe | 3-5 hr |  | Moderate |

| Primary Category | Also Works As | Prep Time | Cook Time | Marinate Time |
|---|---|---:|---:|---:|
| Appetizer | Side | 40 min | 20 min | 2-4 hr |

## Ingredient List

| Component | Ingredients |
|---|---|
| Lemongrass Chicken | Chicken thighs, ginger nuoc cham, lemongrass paste, garlic, neutral oil |
| Wrappers | Rice paper wrappers |
| Filling | Sliced lemongrass chicken, napa cabbage, cucumber, carrot, cilantro, angel hair pasta |
| Sauces | Peanut dipping sauce, ginger nuoc cham |

## Measured Ingredients

### Lemongrass Chicken Marinade

| Amount | Ingredient | Notes |
|---:|---|---|
| 3 small | Boneless chicken thighs |  |
| 2 Tbsp | Restaurant-Style Ginger Nuoc Cham | See condiment link below |
| 1 Tbsp | Lemongrass paste |  |
| 1 clove | Garlic | Minced |
| 1 tsp | Neutral oil |  |

### Spring Roll Components

| Amount | Ingredient | Notes |
|---:|---|---|
| As needed | Rice paper wrappers |  |
| As needed | Sliced lemongrass chicken | Cooked, rested, and thinly sliced |
| As needed | Shredded napa cabbage |  |
| As needed | Julienned cucumber |  |
| As needed | Julienned carrot |  |
| As needed | Fresh cilantro |  |
| As needed | Cooked and cooled angel hair pasta |  |
| For serving | Peanut Dipping Sauce |  |
| For serving | Restaurant-Style Ginger Nuoc Cham |  |

## Instructions

| Step | Action |
|---:|---|
| 1 | Combine chicken thighs, ginger nuoc cham, lemongrass paste, minced garlic, and neutral oil. Marinate 2-4 hours. |
| 2 | Cook chicken until done. Rest, then slice thinly. |
| 3 | Cook angel hair pasta, drain, and cool. |
| 4 | Prep fillings: shred napa cabbage, julienne cucumber and carrot, and gather cilantro. |
| 5 | Soften rice paper wrappers one at a time according to package directions. |
| 6 | Fill each wrapper with chicken, cabbage, cucumber, carrot, cilantro, and cooled angel hair pasta. |
| 7 | Roll tightly and repeat with remaining wrappers and filling. |
| 8 | Serve with peanut dipping sauce and ginger nuoc cham. |

<!-- recipe-card:end -->

## Pairs Well With

| Type | Pairing | Link | Notes |
|---|---|---|---|
| Condiment | Peanut Dipping Sauce | `../../../condiments/peanut-sauce.md` | Primary dipping sauce |
| Condiment | Restaurant-Style Ginger Nuoc Cham | `../../../condiments/ginger-nuoc-cham.md` | Bright dipping sauce and marinade component |
| Main Dish | Lemongrass Chicken Bowls | `../main-dishes/lemongrass-chicken-bowls.md` | Uses the same chicken and sauce set |

## Chef's Notes

This recipe is best treated as a component-sharing recipe with Lemongrass Chicken Bowls. The chicken, sauces, vegetables, and pasta can be prepped once and used for both.

The source recipe does not define the number of rice paper wrappers or exact filling amounts, so this file preserves the spring rolls as flexible, assembly-based.

Because fresh spring rolls can function as either an appetizer or a side, the recipe keeps one canonical file but includes both categories in the metadata for future site browsing.

## Final Verdict

**Worth making again?**

To be determined after testing.

**Flavorweaver Decision:**

- [ ] Keep as written
- [ ] Keep with changes
- [ ] Move toward Hall of Fame
- [ ] Mark as House Staple
- [x] Keep for testing
- [ ] Do not repeat

<details>
<summary>Flavor Architecture</summary>

| Layer | Notes |
|---|---|
| Freshness | Cucumber, carrot, napa cabbage, cilantro. |
| Acid / Brightness | Ginger nuoc cham and lime-forward peanut sauce. |
| Richness | Peanut dipping sauce. |
| Aroma | Lemongrass, ginger, garlic, cilantro. |
| Texture | Soft rice paper, tender chicken, crisp vegetables, soft pasta. |
| Flexibility | Filling amounts can scale based on wrapper size and available vegetables. |

</details>

<details>
<summary>Timing Notes</summary>

Timing includes chicken marination, chicken cooking, pasta cooking, vegetable prep, wrapper softening, rolling, and sauce setup. It does not include making the sauces from scratch if they are not already prepared.

</details>

<details>
<summary>Testing Notes</summary>

## First Test

**Date:**  

**What worked:**

- 
- 
- 

**What to adjust next time:**

- Confirm final yield / number of rolls.
- Confirm ideal pasta amount per roll.
- Note best wrapper handling method.

</details>

<details>
<summary>Kitchen Notes</summary>

Add rolling tips, prep notes, and future refinements here.

</details>

<details>
<summary>Change Log</summary>

| Date | Change |
|---|---|
| 2026-06-11 | Created appetizer recipe file from Lemongrass Chicken Bowls and Fresh Spring Rolls source recipe. |
| 2026-06-11 | Added dual-category metadata so the recipe can appear as both appetizer and side in the future site. |

</details>
